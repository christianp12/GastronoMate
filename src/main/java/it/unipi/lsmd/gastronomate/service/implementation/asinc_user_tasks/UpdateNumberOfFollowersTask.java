...
public class UpdateNumberOfFollowersTask extends Task {
    ...
    public UpdateNumberOfFollowersTask(String username) {
        super(5);
    ...
    public void executeJob() throws BusinessException {
        try{
            Integer followers = neo4jUserDAO.getNumOfFollowers(username);

            System.out.println("Followers: " + followers + " for user: " + username);

            mongoDbUserDAO.updateFollowers(username, followers);

        }catch (DAOException e){
            applicationLogger.severe("Error while updating number of followers: " + e.getMessage());
            throw new BusinessException("Error while updating number of followers: " + e.getMessage(), BusinessTypeErrorsEnum.DATABASE_ERROR);
        }
